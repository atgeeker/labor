    ...
    (function ($) {
        ...
        BootstrapTable.DEFAULTS = {
            ...
            icons: {
                ...
                exportExcel: 'glyphicon-export icon-share'
            },
        ...
        BootstrapTable.prototype.initHeader = function () {
            ...
            $.each(this.options.columns, function (i, column) {
                ...
                style += sprintf('max-width: %s%s; ', column.checkbox || column.radio ? 36 : width, unitWidth);
                ...
                that.header.dicts.push(column.dict);
        ...
        BootstrapTable.prototype.initToolbar = function () {
            ...
            if (this.options.showExportExcel) {
                html.push(sprintf('<button class="btn btn-default' + (this.options.iconSize === undefined ? '' : ' btn-' + this.options.iconSize) + '" type="button" name="exportExcel" title="%s">',
                        this.options.formatExportExcel()),
                    sprintf('<i class="%s %s"></i>', this.options.iconsPrefix, this.options.icons.exportExcel),
                    '</button>');
            }
            ...
            if (this.options.showExportExcel) {
                this.$toolbar.find('button[name="exportExcel"]')
                    .off('click').on('click', $.proxy(this.exportExcel, this));
            }

            if (this.options.search) {
                html = [];
                if(this.options.searchTip == '')
                    html.push('<div class="pull-' + this.options.searchAlign + ' search">',
        ...
        BootstrapTable.prototype.initBody = function (fixedScroll) {
            ...
            for (var i = this.pageFrom - 1; i < this.pageTo; i++) {
                ...
                $.each(this.header.fields, function (j, field) {
                    ...
                    var dict = that.header.dicts[j];
                    if(dict && dictMap[dict]){
                        value = dictMap[dict][value];
                    }
        ...
        BootstrapTable.prototype.initServer = function (silent, query) {
            ...
            $.each(this.header.dicts,function(i,dict){
                if(dict){
                    $.post(that.options.appContext+'/dict/show/'+dict, function(data){
                        var kv = {};
        ...
        BootstrapTable.prototype.refresh = function (params) {
            ...
            //modify by xuqingfeng, 2015-08-26
            if(params && params.query && params.query.pageNumber){
                this.options.pageNumber = params.query.pageNumber;
            }
            this.initServer(params && params.silent, params && params.query);
        };

        //modify by xuqingfeng, 2015-11-18, add function to export excel
        BootstrapTable.prototype.exportExcel = function (query){
            var form = $("<form>");
            form.attr('style', 'display:none');
            form.attr('target','');
            form.attr('method','post');
            form.attr('action',this.options.appContext+'/excel/export');

            $('body').append(form);

            var inputUrl = $('<input>');
            inputUrl.attr('type', 'hidden');
            inputUrl.attr('name', 'url');
            inputUrl.attr('value',this.options.url);
            form.append(inputUrl);

            var inputTitle = $('<input>');
            inputTitle.attr('type', 'hidden');
            inputTitle.attr('name', 'title');
            inputTitle.attr('value',this.options.exportTitle);
            form.append(inputTitle);

            var inputPagination = $('<input>');
            inputPagination.attr('type', 'hidden');
            inputPagination.attr('name', 'pagination');
            inputPagination.attr('value',this.options.pagination);
            form.append(inputPagination);

            var inputSidePagination = $('<input>');
            inputSidePagination.attr('type', 'hidden');
            inputSidePagination.attr('name', 'sidePagination');
            inputSidePagination.attr('value',this.options.sidePagination);
            form.append(inputSidePagination);

            var inputColumns = $('<input>');
            inputColumns.attr('type', 'hidden');
            inputColumns.attr('name', 'columns');
            var columns = this.options.columns;
            var showColumns = new Array();
            for(var i in columns){
                var column = columns[i];
                if(column.title && column.field && column.exportable != false){
                    var temp = {};
                    temp['title'] = column.title;
                    temp['field'] = column.field;
                    temp['dict'] = column.dict;
                    showColumns.push(temp);
                }
            }
            inputColumns.attr('value',JSON.stringify(showColumns));
            ...
            inputParams.attr('type', 'hidden');
            inputParams.attr('name', 'queryParams');
            inputParams.attr('value',JSON.stringify(data));
